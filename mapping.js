...
const KEYWORD_RULES_MULTI = [
  ...
  // 「据上次随访后，是否已按时购药？」在多个模板中出现
  ["purchased_on_time", ["是否已按时购药", "已按时购药"]],
  // 「是否判定患者属于易脱落（失访/停药）高风险？」等
  ["is_dropout", ["是否判定患者属于易脱落", "是否判定患者属于易脱落（失访/停药）高风险"]],
  ["dropout_reason", ["判断为易脱落的原因"]],
  // 「还在按照处方继续使用百悦泽/百泽安」
  ["still_using", ["还在按照处方继续使用"]],
  ["near_usage", ["患者近一次用药情况"]],
  ["delay_reason", ["推迟用药的原因", "延迟用药的具体原因"]],
...
function RE_NONSTD_TXT() { return /减量|减药|延迟|推迟|延后|未按时|漏服|漏药|自行|不规范|不依从|经济|负担|买不起|吃不起/; }
function RE_NORMAL_TXT() { return /规范用药|正常用药|按时|足量|按医嘱|规律用药|继续使用|正在使用/; }
...
function followupSignal(fu) {
  ...
  if (ontime) {
    if (/^(是|已)/.test(ontime)) parts.push("已按时购药");
    else if (/^(否|未)/.test(ontime)) return { signal: "nonstd", reason: "未按时购药" };
...
if (typeof window !== "undefined") {
  window.Mapping = {
    ...
    isPlaceholder, followupSignal, KEYWORD_RULES, KEYWORD_RULES_MULTI,
  };
}
})();
