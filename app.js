...
async function doExport(desen) {
  ...
  for (const r of rows) {
    const vals = effCols.map(([k]) => {
      ...
      // 脱敏导出与界面三组开关保持一致：该字段开关为「不脱敏」时导出明文（与列表所见即所得）
      if (k === "patient_name") {
        if (!desen || state.plainName) return r.patient_name || "";
        if (state.maskMode === "id") return r.member_id || "";
        return maskName(r.patient_name);
      }
      if (k === "phone") {
        if (!desen || state.plainPhone) return r.phone || "";
        return maskPhone(r.phone);
      }
      if (k === "physician") {
        if (!desen || state.plainDoctor) return r.physician || "";
...
$("#exportDesenBtn").onclick = () => {
  // 「脱敏导出」会跟随界面三组开关；若存在「不脱敏」字段，导出物中会含明文，先提示
  const plain = [];
...
window.AppCore = { loadSnapshot, buildRows, filterRows, buildSummary, doExport, disp, STORE, state, DATA, refresh, ensureStdCycles, renderCycleInputs, renderTable, renderPagination, buildSummaryStats, buildSummaryText, renderSummaryPanel, getWeekRange, renderWeekBar, refToday, WEEK_LABEL, classifyFuReason: M.classifyFuReason, DEFAULT_REASON_TREE, cloneReasonTree, flattenReasonTree, reasonLabel, renderReasonManager, addReason, addReasonChild, renameReason, removeReason };
})();
