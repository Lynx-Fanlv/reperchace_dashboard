...
function buildRows() {
  ...
  for (const k in patMap) {
    ...
    for (const fam in p.byProduct) {
      ...
      // 已回购判定：以上一次应购日为基准——倒数第二次购药的应购日已过，
      // 且最近一次购药发生在其后 → 该轮应购已完成（患者实际已回购）。
      // 注意不能用「最近购药+周期」反推（那样永远为 false）。
      let repurchased = false;
      if (bp.dates.length >= 2) {
        const prevPurchase = bp.dates[bp.dates.length - 2];
        const prevDue = addDays(prevPurchase, cycle);
        if (prevDue < today && lastPurchase >= prevDue) repurchased = true;
      }
      ...
      rows.push({
        ...
        last_purchase: lastPurchase, cycle_days: cycle, due_date: dueDate,
        days_to_due: days,
        repurchased,
...
function filterRows(rows) {
  ...
  // 「已逾期」不受时间窗影响（逾期患者始终保留，便于跟进），只影响 应回购/已回购/未到期。
...
function rowHtml(r, idx, inRng, cols) {
  ...
  if (days > 0) dueTxt += `<span class="sub due">还有${days}天</span>`;
  ...
  const cells = cols.map(([key]) => {
    ...
    if (key === "days_to_due") return `<td>${esc(days === 0 ? "今天" : (days > 0 ? "+" + days : String(days)))}</td>`;
    if (key === "status") {
...
async function doSnapshot(desen) {
  ...
  try {
    ...
    // 快照为只读视图，剥离外链库（xlsx / exceljs），避免 file:// 下加载失败
